...
def remove_pc_comp(
    ...
    pca = PCA(n_components=conc_map.shape[-1] - 1).fit(
        np.nan_to_num(conc_map[dummy_mask])
    )
...
def plot_decomp(
    ...
    for i in range(len(comps)):
        ...
        if smooth is not None:
            img = ax[i][0].imshow(
                gaussian_filter(scores[:, :, i], mask, smooth),
                interpolation="none",
                vmin=vmin,
                vmax=vmax,
            )
...
def decompose(
    ...
    if len(data.shape) == 3:
        ...
        raise ValueError(
            "Input array needs to have 2 or 3 dimensions or be Pandas dataframe."
        )
    ...
    if method.lower() == "pca":
        ...
        array, params = feature_normalisation(
            array, return_params=True, mean_norm=False
        )
        array = np.nan_to_num(array)
