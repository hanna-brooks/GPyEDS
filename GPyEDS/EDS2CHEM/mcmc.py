...
class logfuncs:
    ...
    def log_prior(self, theta: list[float]) -> t.Any:
        ...
        if (
            self.pmin[0] < ax < self.pmax[0]
            and self.pmin[1] < ay < self.pmax[1]
            and self.pmin[2] < bx < self.pmax[2]
            and self.pmin[3] < by < self.pmax[3]
            and self.pmin[4] < ww < self.pmax[4]
            and self.pmin[5] < m < self.pmax[5]
            and self.pmin[6] < b < self.pmax[6]
            and self.pmin[7] < phi < self.pmax[7]
        ):
            return 0.0
        return -np.inf
...
def MCMC_run(  # type: ignore
    ...
    with Pool() as pool:
        sampler = emcee.EnsembleSampler(
            nwalkers,
            ndim,
            funcs.log_probability,
            args=(x, y, uncert, positions),
            backend=backend,
            pool=pool,
        )
